...
+2ᵇ : ∀ x y → x + 2ᵇ y ≡ add₁ x (1ᵇ y)
+2ᵇ 0ᵇ y = refl
+2ᵇ (1ᵇ x) y = refl
+2ᵇ (2ᵇ x) y = refl

1ᵇ-double : ∀ x → 1ᵇ x ≡ inc (double x)
1ᵇ-double 0ᵇ = refl
1ᵇ-double (1ᵇ x) = cong 1ᵇ_ (1ᵇ-double x)
1ᵇ-double (2ᵇ x) = refl

2ᵇ-double : ∀ x → 2ᵇ x ≡ double (inc x)
2ᵇ-double 0ᵇ = refl
2ᵇ-double (1ᵇ x) = refl
2ᵇ-double (2ᵇ x) = cong 2ᵇ_ (2ᵇ-double x)

add-inc : ∀ x → add₁ x 0ᵇ ≡ inc x
add-inc 0ᵇ = refl
add-inc (1ᵇ x) = refl
add-inc (2ᵇ x) = refl

+2ᵇ₂ : ∀ x y → add₁ x (2ᵇ y) ≡ add₂ x (1ᵇ y)
+2ᵇ₂ 0ᵇ y = refl
+2ᵇ₂ (1ᵇ x) y = refl
+2ᵇ₂ (2ᵇ x) y = refl

+1ᵇ₂ : ∀ x y → add₁ x (1ᵇ y) ≡ add₂ x (double y)
+1ᵇ₂ 0ᵇ 0ᵇ = refl
...
2ᵇ-double-+₂ : ∀ x y → 2ᵇ add₁ x y ≡ double (add₂ x y)
...
2ᵇ-double-+ : ∀ x y → 2ᵇ (x + y) ≡ double (add₁ x y)
...
*-cong (1ᵇ xs) ys =
  ...
  ⟦ ys ⇓⟧ ℕ.+ ⟦ xs ⇓⟧ ℕ.* 2 ℕ.* ⟦ ys ⇓⟧ ∎
*-cong (2ᵇ xs) ys =
  ⟦ ys +² (ys * xs) ⇓⟧ ≡⟨ cong ⟦_⇓⟧ (+²-lemma ys (ys * xs)) ⟩
