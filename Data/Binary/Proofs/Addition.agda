...
mutual
  ...
  carry-homo : ∀ x₀ xs → ⟦ carry x₀ xs ⇓⟧₁ ≡ suc (2^ suc x₀ * ⟦ xs ⇓⟧₁)
  ...
  1→⟨0+0⟩-homo zero xs (suc y₀) ys = {!!}
  1→⟨0+0⟩-homo (suc x₀) xs zero ys = {!!}
  ...
  1→⟨1+1⟩-homo zero xs zero ys = {!!}
  1→⟨1+1⟩-homo zero xs (suc y₁) ys = {!!}
  1→⟨1+1⟩-homo (suc x₁) xs zero ys = {!!}
